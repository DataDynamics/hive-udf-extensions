 ...
 * <h3>관련 함수</h3>
 * <ul>
 *   <li>SDO_GEOM.SDO_MIN_MBR_ORDINATE (Oracle) - MBR의 최솟값 반환</li>
 * </ul>
 *
 * @see Envelope
 ...
 */
public class SDO_MaxMbrOrdinate extends GenericUDF {

    private transient BinaryObjectInspector geomOI;
    private transient IntObjectInspector ordinatePosOI;

    @Override
    public ObjectInspector initialize(ObjectInspector[] arguments) throws UDFArgumentException {
        if (arguments.length != 2) {
            throw new UDFArgumentException("SDO_MaxMbrOrdinate requires 2 arguments");
        }
        if (!(arguments[0] instanceof BinaryObjectInspector)) {
            throw new UDFArgumentException("SDO_MaxMbrOrdinate requires a binary argument as the first parameter");
        }
        if (!(arguments[1] instanceof IntObjectInspector)) {
            throw new UDFArgumentException("SDO_MaxMbrOrdinate requires an int argument as the second parameter");
        }
        this.geomOI = (BinaryObjectInspector) arguments[0];
        this.ordinatePosOI = (IntObjectInspector) arguments[1];
    ...
    public Object evaluate(DeferredObject[] arguments) throws HiveException {
        ...
        Object ordinatePosObj = arguments[1].get();

        BytesWritable geomBytes = geomOI.getPrimitiveWritableObject(geomObj);
        Integer ordinatePos = (ordinatePosObj == null) ? null : ordinatePosOI.get(ordinatePosObj);
    ...
    public String getDisplayString(String[] children) {
        return getStandardDisplayString("SDO_MaxMbrOrdinate", children);
    }
}