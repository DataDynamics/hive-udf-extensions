 ...
 *   <tr><td>1</td><td>X축</td><td>MBR의 최대 X 좌표 (MinX)</td></tr>
 *   <tr><td>2</td><td>Y축</td><td>MBR의 최대 Y 좌표 (MinY)</td></tr>
 * </table>
 *
 ...
 */
public class SDO_MinMbrOrdinate extends ST_GeometryAccessor {
 
    static final Logger LOG = LoggerFactory.getLogger(SDO_MinMbrOrdinate.class.getName());
        ...
        try {
            ...
            if (ordinatePos == 1) {
                ...
                if (ordinatePos == 3 && esriGeom.hasZ()) {
                    ...
                    return getMinZ(esriGeom);
                } else if (ordinatePos == 4 && esriGeom.hasM()) {
                    return getMinM(esriGeom);
                }
                return null;
            }
        } catch (Exception e) {
            LOG.error("Error in SDO_MinMbrOrdinate: " + e.getMessage(), e);
            return null;
        }
    }
 
    private Double getMinZ(Geometry esriGeom) {
        // ESRI Geometry doesn't provide minZ in Envelope2D.
        ...
        if (esriGeom instanceof com.esri.core.geometry.MultiVertexGeometry) {
            ...
            for (int i = 0; i < mvGeom.getPointCount(); i++) {
                minZ = Math.min(minZ, mvGeom.getPoint(i).getZ());
            }
            return minZ == Double.POSITIVE_INFINITY ? null : minZ;
    ...
    private Double getMinM(Geometry esriGeom) {
        if (esriGeom instanceof com.esri.core.geometry.MultiVertexGeometry) {
            com.esri.core.geometry.MultiVertexGeometry mvGeom = (com.esri.core.geometry.MultiVertexGeometry) esriGeom;
            double minM = Double.POSITIVE_INFINITY;
            for (int i = 0; i < mvGeom.getPointCount(); i++) {
                minM = Math.min(minM, mvGeom.getPoint(i).getM());
            }
            return minM == Double.POSITIVE_INFINITY ? null : minM;
        } else if (esriGeom instanceof com.esri.core.geometry.Point) {
